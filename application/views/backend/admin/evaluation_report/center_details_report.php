...
<?php
	...
	<table border="1" style="width: 1020px;padding: 10px;text-align: center;">
		...
		<th width="" style="border-bottom: 1px solid white">Employee ID</th>
		...
		<?php
			...
			$sql_score = "SELECT * FROM `employee_score` WHERE `employee_id` = '$row_employee[employee_id]' AND  `center_id` = $_POST[center_id] AND `date` BETWEEN '$from_date' AND '$to_date' ";
			...
			if(mysqli_num_rows($result_score) > 0)
			{
				...
				$sql_dehavior = "SELECT * FROM `behavior` WHERE `employee_id` = '$row_employee[employee_id]' AND `center_id` = $_POST[center_id] AND `date` BETWEEN '$from_date' AND '$to_date' ";
			...
			?>
				...
				<tr>
					...
					<td rowspan="4"><?php echo $row_employee['employee_id'];?></td>
					<th rowspan="4">
					<!-- Employee Name -->
					<a target="_blank" style="color:#000;" href="<?php echo site_url('report_controller/employee_performance_details/' . $row_employee['employee_id'] .'/'.$_POST['from_date']. '/' . $_POST['to_date'].'/'.$_POST['center_id']);?>">
    					<?php
